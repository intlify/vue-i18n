...
function localize(context, key, ...args) {
    ...
    if (!isString(format)) {
        ...
        if (missing !== null) {
            ...
            if ((process.env.NODE_ENV !== 'production') && isLocalizeMissingWarn(missingWarn, key)) {
                warn(`Cannot localize the value of '${key}'. Use the value of key as default.`);
            }
            ret = key;
        }
        // falbacking ...
        if ((process.env.NODE_ENV !== 'production') && isLocalizeFallbackWarn(fallbackWarn, key, _fallbackLocaleStack)) {
