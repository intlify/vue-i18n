...
「[異なる配布ファイル](/jp/guide/extra/dist#from-cdn-or-without-a-bundler)」セクションで説明されているように、Vue I18n はバンドラ用に以下の 2 つのビルド済み ES モジュールを提供しています。
...
- `__VUE_I18N_LEGACY_API__`（有効/無効、vue-i18n レガシースタイル API のサポート、デフォルト：`true`）
