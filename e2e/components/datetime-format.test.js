;['composable', 'legacy'].forEach(pattern => {
  describe(`${pattern}`, () => {
    ...
    test('rendering', async () => {
      ...
      await expect(page).toMatch(
        /令和([1-9]|1[0-2])年([1-9]|1[0-2])月([1-9]|[1-3][0-9])日(月|火|水|木|金|土|日)曜日 (午前|午後)(0[0-9]|1[0-2]):([0-5][0-9]):([0-5][0-9]) 日本標準時/
      )
      await expect(page).toMatch(
        /R([1-9]|1[0-2])年([1-9]|1[0-2])月([1-9]|[1-3][0-9])日(月|火|水|木|金|土|日)曜日 (午前|午後)(0[0-9]|1[0-2]):([0-5][0-9]):([0-5][0-9]) 日本標準時/
      )
    })
  })
})
