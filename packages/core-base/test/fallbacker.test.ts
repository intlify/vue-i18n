...
import type { CoreContext, CoreInternalContext } from '../src/context'
...
describe('fallbackWithLocaleChain', () => {
  ...
  describe('cache invalidation', () => {
    ...
    test('updateFallbackLocale() also invalidates the root context', () => {
      ...
      const fallback: Record<string, string[]> = { sl: ['en'], default: ['en'] }
      expect(fallbackWithLocaleChain(root, fallback, 'sl')).toEqual(['sl', 'en'])

      fallback.sl = ['de']
      updateFallbackLocale(local, 'sl', fallback)

      expect(fallbackWithLocaleChain(root, fallback, 'sl')).toEqual(['sl', 'de', 'en'])
    })
  })
})
