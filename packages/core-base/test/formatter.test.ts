...
const formatContracts = [
  {
    ...
    format: (
      ...
      overrides
        ? datetime(context, date, key, overrides)
        : datetime(context, date, key),
    cache: (context: any) =>
      (context as CoreInternalContext).__datetimeFormatters,
    clear: (context: any, formats: any) =>
      clearDateTimeFormat(context, 'en-US', formats)
  },
  {
    name: 'number',
    type: 'number format',
    ...
    format: (
      ...
      overrides
        ? number(context, 1000, key, overrides)
        : number(context, 1000, key),
    cache: (context: any) =>
      (context as CoreInternalContext).__numberFormatters,
    clear: (context: any, formats: any) =>
      clearNumberFormat(context, 'en-US', formats)
  }
] as const
...
describe.each(formatContracts)('$name formatting pipeline', contract => {
  ...
  test('reuse cached formatters and partition caches by overrides', () => {
    ...
    expect(
      cache.has(`en-US__target__${JSON.stringify(contract.overrides)}`)
    ).toBe(true)
  })
