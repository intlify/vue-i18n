...
function sanitizeAttributeValue(attrName: string, value: string): string {
  ...
  const sanitizedValue =
    attrName.toLowerCase() === 'style' ? sanitizeStyleValue(value) : value
...
export function sanitizeTranslatedHtml(html: string): string {
  ...
  html = html.replace(
    ...
    (_, attrName, attrValue) =>
      `${attrName}="${sanitizeAttributeValue(attrName, attrValue)}"`
  )
  ...
  html = html.replace(
    ...
    (match, prefix, attrValue) =>
      hasJavascriptScheme(attrValue) ? `${prefix}about:blank` : match
  )
