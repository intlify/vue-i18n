...
describe('messageResolver', () => {
  ...
  test('fallback', () => {
    ...
    expect(mockMessageResolver).toHaveBeenCalledTimes(6)
    // t('path.to.message'): en (not found) -> ja (found)
    ...
    // te('path.to.message'): en (not found) -> ja (found)
    expect(mockMessageResolver.mock.calls[2][0]).toEqual({})
    expect(mockMessageResolver.mock.calls[2][1]).toEqual('path.to.message')
    expect(mockMessageResolver.mock.calls[3][0]).toEqual(ja)
    expect(mockMessageResolver.mock.calls[3][1]).toEqual('path.to.message')
    // tm('api.errors'): en (not found) -> ja (found)
    expect(mockMessageResolver.mock.calls[4][0]).toEqual({})
    expect(mockMessageResolver.mock.calls[4][1]).toEqual('api.errors')
    expect(mockMessageResolver.mock.calls[5][0]).toEqual(ja)
    expect(mockMessageResolver.mock.calls[5][1]).toEqual('api.errors')
  })
})
