...
describe('useI18n', () => {
  ...
  describe('isolated scope', () => {
    ...
    test('coexists with local scope', async () => {
      ...
      let composableResult: { status: string } | undefined
      const App = defineComponent({
        setup() {
    ...
    test('throws error in Legacy API mode', async () => {
      const i18n = createI18n({
        // legacy: true is the default in v11
        locale: 'en',
        messages: {
          en: { hello: 'hello!' }
        }
      ...
      expect(error).toEqual(
        errorMessages[I18nErrorCodes.NOT_AVAILABLE_COMPOSITION_IN_LEGACY]
      )
    })
  })
})
